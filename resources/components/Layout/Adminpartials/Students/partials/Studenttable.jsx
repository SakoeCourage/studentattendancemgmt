...
function Studenttable({ data, isLoading, fetchData, filters, fullUrl, setViewById ,availablePrograms }) {
                ...
                <div className="flex flex-col w-full min-h-[36rem] h-max relative ">
                    <div className="flex items-center gap-3 w-full justify-end lg: px-10">
                        <div className='border rounded-lg flex items-center justify-between !w-96'>
                            <input onKeyDown={(e) => { e.key === 'Enter' && handleSearch() }} className='bg-transparent outline-none px-4 w-full' placeholder='Search by student id' value={searchKey} onChange={(e) => setSearchKey(e.target.value)} type="search" />
                        ...
                        <FormInputSelect className=" min-w-[15rem]" value={filters?.program} options={Boolean(availablePrograms.length) ? [...availablePrograms.map(p => { return ({ name: p.program_name, value: p.program_name }) })] : []}   onChange={(e) => { fullUrl && fetchData(addOrUpdateUrlParam(fullUrl, 'program', e.target.value)) }} label='Filter Program Name' />
                    
                        {(filters?.search || filters?.program) != null && <Button
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-2 !text-xs flex items-center gap-2 whitespace-nowrap">
                                                        <Tooltip title="Edit Student Data" arrow TransitionComponent={Zoom}>
                                                            <span
                                                                onClick={() => setViewById({ id: x.id, student_id: x.student_id })}
